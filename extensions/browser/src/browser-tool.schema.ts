...
import { isPotatoModeEnabled } from "../../../src/agents/potato-mode.js";
...
export function createPotatoBrowserToolSchema() {
  return Type.Object({
    action: stringEnum(["navigate", "act", "snapshot", "text", "tabs", "close", "open"] as const),
    url: Type.Optional(Type.String({ description: "URL to open." })),
    targetId: Type.Optional(Type.String({ description: "Target tab ID or handle." })),
    ref: Type.Optional(Type.String({ description: "Element ref (e.g. e1) from snapshot." })),
    kind: Type.Optional(stringEnum(["click", "type", "press", "hover", "scroll", "wait"] as const)),
    text: Type.Optional(Type.String({ description: "Text to type." })),
    key: Type.Optional(Type.String({ description: "Key name to press." })),
    query: Type.Optional(Type.String({ description: "Search query to filter snapshot elements." })),
    maxChars: Type.Optional(Type.Number({ description: "Max characters of text to return." })),
  });
}
...
export function createBrowserToolSchema(capabilities: BrowserToolCapabilities, potatoMode?: boolean) {
  if (potatoMode || isPotatoModeEnabled({ potatoMode })) {
    return createPotatoBrowserToolSchema();
  }
