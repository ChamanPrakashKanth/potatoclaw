...
export function describeBrowserTool(opts: {
  ...
  if (opts.potatoMode || isPotatoModeEnabled({ potatoMode: opts.potatoMode })) {
    return "Control the browser. Actions: navigate (url), act (click/type with ref), snapshot (view elements), text (read page text), tabs (list tabs), close.";
  }
