...
export function wrapBrowserExternalText(params: {
  ...
  const isPotato =
    params.potatoMode === true || isPotatoModeEnabled({ potatoMode: params.potatoMode });
  const potatoMaxChars = isPotato ? 600 : DEFAULT_MAX_LIVE_TOOL_RESULT_CHARS;
  ...
  let maxInnerChars = Math.max(
    0,
    Math.min(params.maxChars ?? potatoMaxChars, potatoMaxChars - wrapperOverhead),
  );
  ...
  if (wrappedText.length > potatoMaxChars) {
    maxInnerChars = Math.max(
      0,
      maxInnerChars - (wrappedText.length - potatoMaxChars),
    );
...
export async function appendNavigatedPageState(params: {
  ...
  if (isPotatoModeEnabled()) {
    // In Potato Mode, keep navigation results clean and bounded (title + url).
