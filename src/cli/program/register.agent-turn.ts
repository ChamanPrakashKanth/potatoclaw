...
export function registerAgentTurnCommand(
  ...
  const agent = program
    ...
    .option(
      "--potato",
      "Enable ultra-low-resource Potato Mode for small local models (~2k-4k context)",
      false,
    )
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.openclaw.ai/cli/agent")}`,
    ...
    .action(async (opts): Promise<void> => {
      ...
      if (opts.potato) {
        const { setGlobalPotatoMode } = await import("../../agents/potato-mode.js");
        setGlobalPotatoMode(true);
      }
  ...
  agent
    ...
    .action(async (message: string | undefined, opts, command): Promise<void> => {
      ...
      const execOpts = {
        ...
        potato: opts.potato === true || (parentOpts as { potato?: boolean } | undefined)?.potato === true,
      };
      if (execOpts.potato) {
        const { setGlobalPotatoMode } = await import("../../agents/potato-mode.js");
        setGlobalPotatoMode(true);
