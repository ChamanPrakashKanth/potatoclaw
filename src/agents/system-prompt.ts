...
import { isPotatoModeEnabled } from "./potato-mode.js";
...
export function buildAgentSystemPrompt(params: {
  ...
  if (
    promptMode === "potato" ||
    isPotatoModeEnabled({ potatoMode: (params as { potatoMode?: boolean }).potatoMode })
  ) {
    const lines = [
      "You are OpenClaw (Potato Mode), an ultra-efficient autonomous agent.",
      modelIdentityLine,
      `Date: ${params.userDate ?? new Date().toISOString().slice(0, 10)}${params.userTimezone ? ` (${params.userTimezone})` : ""}`,
    ];
    if (params.workspaceDir) {
      lines.push(`Workspace: ${sanitizeForPromptLiteral(params.workspaceDir)}`);
    }
    lines.push(
      "",
      "Directives:",
      "- Answer directly and concisely. Avoid filler or chit-chat.",
      "- Use available tools to accomplish the goal immediately.",
      "- Summarize final outcomes clearly.",
    );
