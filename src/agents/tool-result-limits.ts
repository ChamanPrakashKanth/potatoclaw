...
export const POTATO_MAX_LIVE_TOOL_RESULT_CHARS = 1_000;
...
export function resolveAutoLiveToolResultMaxChars(
  contextWindowTokens: number,
  potatoMode?: boolean,
): number {
  if (potatoMode === true || isPotatoModeEnabled({ potatoMode })) {
    return POTATO_MAX_LIVE_TOOL_RESULT_CHARS;
  }
...
export function calculateMaxToolResultCharsWithCap(
  ...
  const isPotato = potatoMode === true || isPotatoModeEnabled({ potatoMode });
  const effectiveCap = isPotato ? Math.min(POTATO_MAX_LIVE_TOOL_RESULT_CHARS, hardCapChars) : hardCapChars;
  const maxTokens = Math.floor(contextWindowTokens * MAX_TOOL_RESULT_CONTEXT_SHARE);
  const maxChars = maxTokens * 4;
  return Math.min(maxChars, Math.max(1, effectiveCap));
}
