...
export { CONTEXT_WINDOW_HARD_MIN_TOKENS_POTATO, CONTEXT_WINDOW_WARN_BELOW_TOKENS_POTATO };
...
function resolveContextWindowGuardThresholds(
  ...
  const hardFloor = isPotato ? CONTEXT_WINDOW_HARD_MIN_TOKENS_POTATO : CONTEXT_WINDOW_HARD_MIN_TOKENS;
  const warnFloor = isPotato ? CONTEXT_WINDOW_WARN_BELOW_TOKENS_POTATO : CONTEXT_WINDOW_WARN_BELOW_TOKENS;
  return {
...
export function formatContextWindowWarningMessage(params: {
  ...
  if (params.guard.source === "modelsConfig") {
    return (
      ...
      `so raise contextWindow/contextTokens if it is set too low (or use Potato Mode for low-VRAM/small models)`
    );
  }
  return (
    `${base}; local/self-hosted runs work best at ` +
    `${params.guard.warnBelowTokens}+ tokens (or enable Potato Mode via potatoMode: true for ~2k-4k models)`
  );
}
...
export function formatContextWindowBlockMessage(params: {
  ...
  if (params.guard.source === "modelsConfig") {
    return (
      `${base} OpenClaw is using the configured model context limit for this model. ` +
      `Raise contextWindow/contextTokens, enable Potato Mode (potatoMode: true / --potato), or choose a larger model.`
    );
  }
  return (
    `${base} This looks like a local model endpoint. ` +
    `Enable Potato Mode (potatoMode: true or --potato) for small local models, ` +
...
export function evaluateContextWindowGuard(params: {
  ...
  const isPotato =
    params.potatoMode === true ||
    isPotatoModeEnabled({ config: params.cfg, potatoMode: params.potatoMode });
  const resolvedThresholds = resolveContextWindowGuardThresholds(referenceTokens, isPotato);
